    ...
    def forward(self, x):
        ...
        for i in range(out_h):
            ...
            for j in range(out_w):
                tmp = x[r: r + W.shape[0], c: c + W.shape[1]]
                out[i, j] = np.sum(tmp * W)
                self.cache.append(tmp.flatten())
                c += self.stride
            r += self.stride
    ...
    def backward(self, dout):
        ...
        for i in range(b):
            ...
            for tmp in self.cache:
                temp.append(tmp[i])
            a.append(np.array(temp).reshape(dout.shape))

        a = np.array(a)

        dW_tmp = np.zeros(a.shape[0])

        for i in range(dW_tmp.shape[0]):
            dW_tmp[i] = np.sum(a[i] * dout)

        dW = dW_tmp.reshape(dW.shape[0], dW.shape[1])
